...
func (e *Env) LastfmLovedTracksHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(ErrorResponse{Error: "Failed to get user's loved tracks: " + err.Error()})
		return
	}
	...
	lovedTracksResp, requestErr := api.GetLovedTracks(limit, page)
	if requestErr != nil {
		w.WriteHeader(requestErr.StatusCode)
		json.NewEncoder(w).Encode(ErrorResponse{Error: "Failed to get user's loved tracks: " + requestErr.Error()})
		return
	}

	json.NewEncoder(w).Encode(lovedTracksResp.LovedTracks)
}
